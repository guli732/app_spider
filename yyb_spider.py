...
class Customer(Thread):
    ...
    def parse_page(self, text, html):
        ...
        try:
            # 应用名称
            appname = html.xpath("//div[@class='det-name-int']/text()")[0]
            print(appname)
            # id
            id = re.findall(r'appId : "(.*?)"', text)[0]
            # 更新时间
            try:
                pub_time = re.findall(r'data-apkPublishTime="(.*?)"', text)[0]
                pub_time = time.strftime('%Y-%m-%d', time.localtime(int(pub_time)))
            except:
                pub_time = ''
            # 标签
            tag = ''
            # 应用包名
            packagename = re.findall(r'apkName : "(.*?)"', text)[0]
            # 应用图标
            try:
                icon_url = html.xpath("//div[@class='det-icon']/img/@src")[0]
            except:
                icon_url = ''
            # 下载量
            try:
                download_count = re.findall(r'downTimes:"(.*?)"', text)[0]
            except:
                download_count = ''
            # 一级分类
            try:
                category = html.xpath("//div[@class='det-type-box']/a/text()")[0]
            except:
                category = ''
            # 二级分类
            category_two = ''
            # 版本号
            try:
                version_number = html.xpath("//div[@class='det-othinfo-container J_Mod']/div[2]/text()")[0]
            except:
                version_number = ''
            # 开发商
            try:
                author_name = html.xpath("//div[@class='det-othinfo-container J_Mod']/div[6]/text()")[0]
            except:
                author_name = ''
            # 应用描述
            try:
                app_disc = html.xpath("//div[@class='det-app-data-info'][1]/text()")[0]
                app_disc = re.sub(r'\n| |\r', '', app_disc)
            except:
                app_disc = ''
            # 创建时间（注意更新数据时不再改动）
    ...
    def save_data(self, app_info):
        with open('app.csv', 'a', encoding='utf8') as fp:
...
def main():
    ...
    for package_name in package_names:
        ...
        url = 'https://sj.qq.com/myapp/detail.htm?apkName={}'.format(package_name)
        queue.put(url)
    # 消费者
    for i in range(4):
